...
public class ModeInputPanel extends GridPane {
    ...
    private void createListView(){
        ...
        selectBoardLabel.setFont(Font.font("Arial Unicode MS", 16));
        
        selectBoardButton.setStyle("-fx-background-color: #17871b; -fx-text-fill: white;");
        selectBoardButton.setPrefSize(200, 50);
        selectBoardButton.setFont(Font.font("Arial Unicode MS", 16));
	...
	ObservableList<String> ListFiles = FXCollections.observableArrayList(files);
	listView.setItems(ListFiles);

	for (String f : ListFiles) {

	    if (f.toString().endsWith("Starter.txt")) {

		return ListFiles.indexOf(f);
	    } // end of if statement
    ...
    /**
     * Loads the board file selected in the boardsList and updates the
     * selectBoardLabel with the name of the new Board file
     * 
     * @param selectBoardLabel a message Label to update which board is currently
     *                         selected
	...
	String filename = boardsList.getSelectionModel().selectedItemProperty().getValue();

	File file = new File(filename);
	selectBoardLabel.setText(String.format("Current board: %s", file.getName()));

	this.view.model = new ThreeMusketeers(file);
